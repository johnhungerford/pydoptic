...
```python3
from pydoptic import BaseModel, Partial, Prop, PropOpt, PropOptArr
from datetime import date
...
```python3
partial_person: Partial[Person] = Person.partial(name="John", birth_date=...)
...
```python3
from pydoptic import Prop, PropOptArr, Partial
...
found_people: List[Partial[Person]] = elastic_service.search_partial(query, source=[Person.name, Person.birth_date])
